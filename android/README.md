...
### 1. 2D 週功課表主畫面與微件 (TimetableGlanceWidget & App MainScreen)
- **左側時間節次軸**：顯示標準節次代碼（1 至 8 / 0 至 D）與開課時間，為右側課表格子提供最大版面。
- **頂部星期欄**：英文簡寫標籤（MON 至 FRI），當日動態切換為深藍色發光底色與啟用點（●）指示。
- **精確跨節對齊**：依每門課的實際起訖節次垂直跨行延展，未排課時段以整齊底格對齊。
- **彩色磨砂卡片**：課程依名稱雜湊產生專屬色彩主題與邊框，標註教室地點（📍），點擊可查看完整授課教師與時段。

### 2. 今日節次功課表微件 (TodayGlanceWidget)
- **節次時間對照**：清楚列出今日各排定課程之節次區間（如第 3-4 節 10:20-12:10）與教室地點。
- **即時上課狀態**：當前上課時段自動醒目標示「上課中」標籤。
- **一鍵開啟課表**：點擊小工具任意位置即可喚起 2D 週功課表。
